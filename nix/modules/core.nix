{ self, lib, ... }: {
  perSystem = { config, self', inputs', pkgs, ... }: {
    ...
    packages = rec {
      hcTest = pkgs.writeShellScriptBin "hc-test" ''
        set -euxo pipefail
      ...
      hcStandardTests = pkgs.writeShellScriptBin "hc-test-standard" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcStandardTestsNextest = pkgs.writeShellScriptBin "hc-test-standard-nextest" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcWasmTests = pkgs.writeShellScriptBin "hc-test-wasm" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcReleaseAutomationTest = pkgs.writeShellScriptBin "hc-test-release-automation" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcReleaseAutomationTestRepo =
        let
          ...
          crateCmd = logLevel: ''
            ${self'.packages.hcReleaseAutomation}/bin/hc-ra \
          ...
          '';
        in
        pkgs.writeShellScriptBin "hc-test-release-automation-repo" ''
          set -euxo pipefail
      ...
      hcStaticChecks =
        let
          ...
          ;
        in
        pkgs.writeShellScriptBin "hc-static-checks" ''
      ...
      hcMergeTest = pkgs.writeShellScriptBin "hc-merge-test" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
        hc-static-checks
        hc-test
      ...
      hcReleaseTest = pkgs.writeShellScriptBin "hc-test-release" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcSpeedTest = pkgs.writeShellScriptBin "hc-speed-test" ''
      ...
      hcFlakyTest = pkgs.writeShellScriptBin "hc-flaky-test" ''
        set -euxo pipefail
        export RUST_BACKTRACE=1
      ...
      hcDoctor = pkgs.writeShellScriptBin "hc-doctor" ''
      ...
      hcBench = pkgs.writeShellScriptBin "hc-bench" ''
        cargo bench --bench bench
      '';

      hcFmtAll = pkgs.writeShellScriptBin "hc-fmt-all" ''
        fd Cargo.toml crates | xargs -L 1 cargo fmt --manifest-path
      '';

      hcBenchGithub = pkgs.writeShellScriptBin "hc-bench-github" ''
        set -x
      ...
      hcRegenReadmes = pkgs.writeShellScriptBin "hc-regen-readmes" ''
        cargo-readme readme --project-root=crates/release-automation/ --output=README.md;
      '';

      hcRegenNixExpressions = pkgs.writeShellScriptBin "hc-regen-nix-expressions" ''
        set -xe
        pushd ${self}
      ...
      hcRunCrate = pkgs.writeShellScriptBin "hc-run-crate" ''
        set -x
        source ${self'.packages.nixEnvPrefixEval}
        ...
        binary=$(cargo build \
          ...
          --manifest-path=${self}/crates/$crate/Cargo.toml \
      ...
      hcCrateBinaryPath = pkgs.writeShellScriptBin "hc-crate-binary-path" ''
        set -x
        source ${self'.packages.nixEnvPrefixEval}
    ...
    // (lib.optionalAttrs pkgs.stdenv.isLinux {
      hcCoverageTest = pkgs.writeShellScriptBin "hc-coverage-test" ''
        set -euxo pipefail
