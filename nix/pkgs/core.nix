...
rec {
  ...
  hcStandardTestsNextest = writeShellScriptBin "hc-test-standard-nextest" ''
    ...
    cargo build --features 'build' -p holochain_wasm_test_utils
    cargo nextest ''${CARGO_NEXTEST_ARGS:-run --test-threads=2} --workspace --features slow_tests,glacial_tests,test_utils,build_wasms,db-encryption''${HC_ADD_FEATURES:-} --lib --tests --cargo-profile fast-test ''${1-}
  '';
