...
Once the walk produces a complete chain from seq 0 to `head_hash`, the workflow writes it directly into the per-DNA database in the same shape that authoring would have produced, except that private entry content is never written as `get_agent_activity` never distributes private entries, see [Scope and assumptions](#scope-and-assumptions). Therefore, a record whose action declares a private entry restores with the action present but no corresponding row in `PrivateEntry`.
