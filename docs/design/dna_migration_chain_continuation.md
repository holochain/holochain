...
the rules *underneath a single, continuous chain*, so the agent's source
chain and network membership are preserved across the migration.
...
— the data model's per-action header (author, timestamp, sequence,
...
end the chain — authorship continues directly after it. The marker itself is the
first action stamped with the new `IntegrityHash`, so it is validated under the
version it adopts, and that version is in force from this action onward. It may also carry an
...
- From a `ContinueChain` onward — the marker included — every action must
  reference the version it declared, until the next `ContinueChain`.
...
Its **purpose** is different: the existing field lives behind
`unstable-migration` and today serves a single consumer — the
`GetCompatibleCells` admin query backing `UseExisting` cell reuse. Continuation
gives it a fuller role: the ordered version history of a lineage.
...
problems: such content cannot be validated, and while it waits it must be kept
**inert** — out of the validation queue and unable to pollute the retry loop.
Hard eviction of content that never becomes validatable is a separate concern,
left to Future options below.
