...
When dealing with distributed systems, however, the application of logical models and formal proofs are often deceiving. This stems from how easy it is to define sets and conditions which are logically solid but fundamentally impossible and unintelligible in practice. Since our primary intent with Holochain is to provide a highly functional and scalable framework for sophisticated decentralized coordination, our focus must be on what is practicable, and resist the pull of theoretical ideals which steer builders into unwieldy architectures.

Note how easy it is to reference something like "all living persons" or "all nodes in the network." But in actuality, it is impossible to identify that set in the real world. Even after eliminating inherent fuzziness in the definition of "living", there is no way to discover and record the information quickly enough about who is dying and being born to construct the actual data set. Likewise, no single agent can know with certainty which nodes have gone offline, come online, or have declared themselves as new nodes, and since network partitions are a real, one must question which network is being referring to when it is partitioned, and whether it is appropriate to limit a node's agency beyond the constraints which the partition naturally impose.

The previous example is a comparatively easy data set to work with, since it changes so slowly -- typically each person undergoes a state change only twice in their life (when they become a living person, and when they cease to be one, for some definition of "living"). The kinds of tools we need for powering the next web need to handle much more rapid and complex changes. A more apt logical construct might be "all people with just one foot on the ground". Membership in this set changes quite rapidly -- around 1/2 billion times per second[footnote].

It should be obvious there is no way in practice to work with that data set without requiring actions that either break reality (like freezing time) or asserting a god-like, omniscient being, who not only has instantaneous access to all knowledge (requiring information propagation faster than the speed of light), but also has infinite attention to all states (likely requiring infinite energy). However, since our current computing architectures are bound by laws of physics, we should avoid the temptation of injecting such impossible constructs into our model. A proof that involves simple logical concepts which cannot be reliably constructed in practice is not much of a proof at all.

"Global state" and strategies for consensus about it are exactly one of these constructs which are easy concepts, but cannot adequately reflect events happening in physical reality without drastic reduction of complexity, agency, and degrees of freedom. Yet most current distributed systems undertake the expensive task of having each node construct and maintain this unwieldy fiction. Hence blockchains may run on thousands of processors, but advance in lockstep as a single-threaded process, and they are mostly reliable for very simplified world models, like moving tokens (subtracting from a number in one address and adding it to the number in another address).
...
Early influential works in decentralized computing (such as the Byzantine Generals' Problem[footnote]) may have also set such expectations. Those papers were written in the context of reaching consensus in finite control systems where there was a known number of sensors and states, and the goal was to reach a unified decision when correct operation could not be guaranteed (like nine generals deciding a time for all to attack). Therefore, to be Byzantine Fault Tolerant[footnote], most distributed systems have assumed[footnote] that global consensus must be constructed in order to reach a unified action. In this paper, we will detail some more efficient paths to enable agents to act which do not require a construct of global consensus at all.

So, while the formalizations from the original Holochain white paper are still valid in theory, this white paper is concerned with proving that those formalizations work in practice. So we will start by stating our underlying assumptions as axioms -- each of which correlates to architectual properties of Holochain. And we will take special care not to make grand, categorical statements which cannot be implemented inside the constraints of computional systems bounded by the laws of physics.

## Axioms -- Our Underlying Basis for Coordination

Here we spell out the assumptions upon which we have built our approach to the challenge of decentralized coordination.
...
3. It is easy to agree on a starting state so that the ground rules for coordination may be established up front.
...
9. Effective coordination arises from the ability to articulate appropriate ground rules, including the ability to change said rules.
...
**Effective coordination arises from the ability to articulate appropriate ground rules, which including the ability to change said rules.** Coordination cannot be effective if it does not comprehend the real-world context it is applying its efforts to. However, agents cannot fully comprehend their context at first; understanding comes with interaction over time. And as the agents interact with and within their context, they change the context, bringing new situations to comprehend. This means that any "grammar" in which ground rules are written must include the capacity to express rules for evolving those same rules in response to changing understanding and context. Furthermore, agents in one group naturally interact with agents in another group. This provides points of divergent and convergent evolution via experimentation and cross-pollination of understanding.

In Holochain, the ground rules for a group are expressed in executable code written in conventional programming languages. Its tools include the ability for agents to migrate to a new group with a new set of rules, as well as the ability for an agent to "bridge" their presence in two different groups via cross-space function calls.

In all these axioms, agents are empowered to take independent and autonomous action with partial information as soon as they have whatever they need to ensure it is a correct action. This constitutes a significant departure from the frame of thinking that Byzantine Fault Tolerance traditionally assumes: that a construction of complete consensus is required *before* an agent can act.
