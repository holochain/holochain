...
9. Effective coordination arises from a grammar which is expressive enough to articulate appropriate and evolvable ground rules.
...
**Agents always act based only on their state; that is, data they can access locally.**

**Effective coordination arises from a grammar which is expressive enough to articulate appropriate and evolvable ground rules.** Coordination cannot be effective if it does not comprehend the real-world context it is applying its efforts to. However, agents cannot fully comprehend their context at first; understanding comes with interaction over time. And as the agents interact with and within their context, they change the context, bringing new situations which must be comprehended. These challenges mean that any "grammar" in which ground rules are written must include the capacity to express rules for evolving those same rules in response to changing understanding and context. Furthermore, groups of agents naturally
