...
See the [Formal Design Elements](hwp_4_formal.md) section for more information on how Agents convert Source Chain data to operations that are published into the collectively stored data on the DHT, and how this works to provide eventual consistency, and the sections in [System Correctness](hwp_3_correctness.md) for details on detection of malicious actors.
