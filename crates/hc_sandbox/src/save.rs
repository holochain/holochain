...
pub fn load(mut hc_dir: PathBuf) -> std::io::Result<Vec<Option<PathBuf>>> {
...
pub async fn lock_live(mut hc_dir: PathBuf, path: &Path, port: u16) -> anyhow::Result<()> {
    ...
    let index = match paths.into_iter().enumerate().find(|p| p.1 == Some(path.to_path_buf())) {
        Some((i, _)) => i,
        None => return Ok(()),
    };
...
pub fn find_ports(hc_dir: PathBuf, paths: &[PathBuf]) -> anyhow::Result<Vec<Option<u16>>> {
    ...
    for path in paths {
        let index = all_paths.iter().position(|p| *p == Some(path.to_path_buf()));
        match index {
            Some(i) => {
                let mut hc = hc_dir.clone();
                hc.push(format!(".hc_live_{}", i));
...
mod tests {
    ...
    fn test_load_valid_paths() -> anyhow::Result<()> {
        ...
        assert_eq!(loaded_paths[0], Some(sandbox1.clone()));
        assert_eq!(loaded_paths[1], Some(sandbox2.clone()));
    ...
    #[test]
    fn test_load_with_missing_paths() -> anyhow::Result<()> {
        // Create a temporary directory for testing
        let temp_dir = tempfile::tempdir()?;
        let test_dir = temp_dir.path();
        let hc_dir = test_dir.join("hc_dir");
        ...
        assert_eq!(loaded_paths.len(), 2);
        assert_eq!(loaded_paths[0], Some(sandbox1.clone()));
        assert_eq!(loaded_paths[1], None); // This path should be None because the directory was removed
    ...
    fn test_load_after_directory_removal() -> anyhow::Result<()> {
        ...
        // Verify the loaded paths
        assert_eq!(loaded_paths.len(), 2);
        assert_eq!(loaded_paths[0], None); // This path should be None because the config file was removed
        assert_eq!(loaded_paths[1], None); // This path should be None because the config file was removed
