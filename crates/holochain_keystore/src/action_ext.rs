...
mod test {
    ...
    /// A `CloseChain` with an agent target must be signed by its author.
    ///
    /// Accepting the target's signature would let it close another agent's
    /// chain in that agent's name (#5981).
    ...
    async fn close_chain_with_agent_target_is_signed_by_author() {
        ...
        let forged_signature = new_agent.sign(&keystore, &action).await.unwrap();
        let forged = SignedActionHashed::with_presigned(hashed, forged_signature);
        assert!(matches!(
