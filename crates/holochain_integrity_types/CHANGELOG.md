...
## Unreleased
- **BREAKING**: Original action and entry have been removed from relevant variants of `Op`. Op is passed into the app validation callback `validate` where validation rules of an app can be implemented. Update and delete operations used to prefetch original action and original entry, regardless of whether they were used in `validate` or not. Particularly for an update or delete of an entry it is not common to employ the original entry in validation. It is therefore removed from those variants of `Op` which means a potential performance increase for not having to fetch original actions and entries for all ops to be validated. To use original action and entry during validation, they can be explicitly fetched with the HDK calls `must_get_action` and `must_get_entry`.
