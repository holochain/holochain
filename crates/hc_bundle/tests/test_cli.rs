...
fn read_app(path: &Path) -> anyhow::Result<AppBundle> {
    Ok(AppBundle::unpack(ffs::sync::read(path).unwrap().into())?)
}

fn read_dna(path: &Path) -> anyhow::Result<DnaBundle> {
    Ok(DnaBundle::unpack(ffs::sync::read(path).unwrap().into())?)
}

fn read_web_app(path: &Path) -> anyhow::Result<WebAppBundle> {
    Ok(WebAppBundle::unpack(ffs::sync::read(path).unwrap().into())?)
}
