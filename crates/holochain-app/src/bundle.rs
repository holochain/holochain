...
use crate::manifest::{AppManifestValidated, AppRoleManifestValidated};
use crate::*;
use futures::future::join_all;
use holochain_dna_types::prelude::*;

pub type AppBundleError = anyhow::Error;
pub type AppBundleResult<T> = Result<T, AppBundleError>;

/// A bundle of an AppManifest and collection of DNAs
#[derive(
    Debug, Serialize, Deserialize, derive_more::From, derive_more::Deref, derive_more::DerefMut,
)]
...
impl AppBundle {
    ...
    async fn resolve_dna(
        ...
        let dna_file = if let Some(hash) = installed_hash {
            ...
            if expected_hash != original_hash {
                anyhow::bail!(
                    "Could not resolve the app role '{role_name}' because of hash mismatch: {original_hash} != {expected_hash}"
                );
            }
            dna_file
        } else {
    ...
    async fn resolve_location(
        ...
        let bundle = mr_bundle::Bundle::<ValidatedDnaManifest>::decode(&bytes)?;
        let dna_bundle = DnaBundle::from(bundle);
...
mod tests {
    ...
    use crate::bundle::AppRoleResolution;
    use crate::manifest::app_manifest_v1::tests::{
        app_manifest_fixture, app_manifest_properties_fixture,
    };
    use crate::manifest::DnaLocation;
    use crate::*;
    use ::fixt::prelude::*;
    use holochain_dna_types::prelude::*;
