...
use holochain_sqlite::{db::default_max_readers, prelude::*};
...
async fn async_read_respects_reader_permit_limits() {
    ...
    let num_short_readers = default_max_readers() / 2;
...
async fn get_read_txn_respects_reader_permit_limits() {
    ...
    let num_long_readers = default_max_readers() / 2;
...
async fn read_async_releases_permits() {
    ...
    assert_eq!(
        ...
        db_handle.available_reader_count()
    );
