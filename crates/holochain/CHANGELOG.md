...
- `GrantZomeCallCapability` now rejects a grant whose capability is not `Capability::ZomeCall`, instead of committing a grant that could never authorize a zome call.
- **BREAKING CHANGE**: Capability grants are restructured so a grant can describe capabilities other than zome calls. `CapGrantEntry` is now `{ tag, constraint, capability }`: `access` is renamed to `constraint`, and `functions` moves inside `capability: Capability::ZomeCall(ZomeCallGrant { functions })` — `CapGrant::new_zome_call_grant` builds that shape. Types are renamed to match: `ZomeCallCapGrant` → `CapGrant`, the old `CapGrant` enum → `CapAccess`, `CapAccess` → `GrantConstraint`, `CapAccessType` → `GrantConstraintType` (now in `holochain_integrity_types::capability`), `DesensitizedZomeCallCapGrant` → `DesensitizedCapGrant`, `CapAccessInfo` → `GrantConstraintInfo`, `Entry::as_cap_grant` → `Entry::as_cap_access`. `CapAccess::is_valid` is split into `is_valid_for_zome_call` and `is_valid_for_direct_signal`, so a grant only authorizes the capability it was issued for. Capability grants already on a source chain cannot be read by this version and must be re-issued, and the `GrantZomeCallCapability` and `ListCapabilityGrants` wire formats change with it.
