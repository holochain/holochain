...
- Crates `holochain_conductor_api`, `holochain_cli_client`, `holochain_conductor_config` and `holochain_client` now have features for the network transport (tx5 or iroh), which are used in `NetworkConfig` and also set which transport is used.
