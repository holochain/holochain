...
## Unreleased
- The app validation workflow had been retriggered only when the number of concurrent ops to be validated (50) was reached. In many scenarios that doesn't happen and thus the workflow wasn't retriggered, even though some ops might not have been validated. Now the workflow will return `WorkComplete::Incomplete` when not all ops could be validated, which retriggers the workflow.
