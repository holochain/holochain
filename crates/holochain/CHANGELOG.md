...
- **BREAKING CHANGE**: `holochain_integrity_types::action::CapAccessType` is renamed to `GrantConstraintType` and moved to `holochain_integrity_types::capability`, next to the `GrantConstraint` enum whose variants it discriminates. It gains a `From<&GrantConstraint>` conversion, which is now the single source of truth for the integer stored in the `CapGrant.cap_access` column; the stored values are unchanged.
