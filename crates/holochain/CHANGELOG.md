...
- **BREAKING CHANGE**: `holochain_integrity_types::action::CapAccessType` is renamed to `GrantConstraintType` and moved to `holochain_integrity_types::capability`, next to the `GrantConstraint` enum whose variants it discriminates. It gains a `From<&GrantConstraint>` conversion, which is now the single source of truth for the integer stored in the `CapGrant.cap_access` column, and loses its unused `TryFrom<i64>` conversion. The stored values are unchanged.
- **BREAKING CHANGE**: The capability grant types are renamed and restructured so that a grant can describe capabilities other than zome calls. `ZomeCallCapGrant` is now `CapGrant` (still the type committed as `CapGrantEntry`), the former `CapGrant` enum of who-is-calling is now `CapAccess`, and the former `CapAccess` enum of `Unrestricted`/`Transferable`/`Assigned` is now `GrantConstraint`. On the grant entry itself, the `access` field is renamed to `constraint`, and the `functions` field is replaced by `capability: Capability`, where `Capability::ZomeCall(ZomeCallGrant { functions })` carries the granted zome/function pairs. `CapGrant::new_zome_call_grant` builds that shape directly. `DesensitizedZomeCallCapGrant` is now `DesensitizedCapGrant` and `CapAccessInfo` is now `GrantConstraintInfo`. `Entry::as_cap_grant` is renamed to `Entry::as_cap_access`, and validity checks are split into `CapAccess::is_valid_for_zome_call` and `CapAccess::is_valid_for_direct_signal` so a grant only authorizes the capability it was issued for. This changes the serialized form of `CapGrantEntry`, so capability grants already committed to a source chain are not readable by this version and must be re-issued; it also changes the wire format of the `GrantZomeCallCapability` admin call and of the grant information returned by `ListCapabilityGrants`.
- **BREAKING CHANGE**: Remove `Entry::app_fancy` and `Entry::entry_type`. Build an `Entry::App` with `Entry::app` after converting to `SerializedBytes` yourself, and read an entry type from the action that wrote it.
