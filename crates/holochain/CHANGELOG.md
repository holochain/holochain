...
- Chore: Update kitsune2 to v0.4.0-dev.2, which includes the iroh relay integration with the bootstrap server.
- Crates `holochain_conductor_api` and `holochain_cli_client` now have features for the network transport (tx5 or iroh), which are used in `NetworkConfig` and also set which transport is used.
