...
impl ConductorBuilder {
    ...
    pub async fn build(self) -> ConductorResult<ConductorHandle> {
        ...
        let net_peer_meta_store_db = spaces.peer_meta_store_db()?;
        let net_spaces = spaces.clone();
        let p2p_config = holochain_p2p::HolochainP2pConfig {
            get_db_peer_meta: Arc::new(move || {
                let peer_meta = net_peer_meta_store_db.clone();
                Box::pin(async move { Ok(peer_meta) })
            }),
            get_db_op_store: Arc::new(move |dna_hash| {
                let res = net_spaces.dht_db(&dna_hash);
