...
mod network_impls {
    ...
    impl Conductor {
        ...
        pub async fn agent_meta_info(
            ...
            // Sort to get consistently ordered JSON output
            space_ids.sort();

            let mut all_infos = serde_json::Map::new();
            ...
            for dna_hash in space_ids {
                ...
                let infos = db
                    .read_async(move |txn| -> DatabaseResult<serde_json::Value> {
                        let mut infos: Vec<serde_json::Value> = Vec::new();

                        let mut stmt = txn.prepare(sql_peer_meta_store::GET_ROW_FOR_URL)?;
                        ...
                        while let Some(row) = rows.next()? {
                            ...
                            let peer_url = row.get::<_, String>(0)?;
                            let meta_key = row.get::<_, String>(1)?;
                            let meta_value: serde_json::Value = serde_json::from_slice(
                                &(row.get::<_, BytesSql>(2)?.0),
                            )
                            .map_err(|e| {
                            ...
                            map.insert("peer_url".into(), peer_url.into());
                            map.insert("meta_key".into(), meta_key.into());
                            map.insert("meta_value".into(), meta_value);
                            map.insert("expires_at".into(), expires_at.into());

                            infos.push(serde_json::Value::Object(map))
                        }

                        Ok(serde_json::Value::Array(infos))
                    })
                    .await?;

                all_infos.insert(DnaHashB64::from(dna_hash).to_string(), infos);
            }
