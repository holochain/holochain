...
mod tests {
    ...
    async fn scenario_smoke_test_single_node() {
        ...
        let scenario = SweetGossipScenario::setup_from_scenario(def, dna_file).await;
        let [node] = scenario.nodes();

        let ops0 = node.get_op_basis_buckets().await;
    ...
    async fn scenario_smoke_test_two_nodes() {
        ...
        let ops0 = node0.get_op_basis_buckets().await;
        let ops1 = node1.get_op_basis_buckets().await;
