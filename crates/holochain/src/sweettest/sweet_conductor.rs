...
impl SweetConductor {
    ...
    pub async fn setup_app_for_agent(
        ...
        let dnas: Vec<_> = dnas.into_iter().collect();
        self.setup_app_1_register_dna(dnas.clone()).await?;
        self.setup_app_2_install_and_enable(installed_app_id, agent.clone(), dnas.clone())
            .await?;
    ...
    pub async fn setup_app_for_agents(
        ...
        self.setup_app_1_register_dna(dnas.clone()).await?;
        for agent in agents.clone().into_iter() {
            let installed_app_id = format!("{}{}", app_id_prefix, agent);
            self.setup_app_2_install_and_enable(&installed_app_id, agent.to_owned(), dnas.clone())
                .await?;
        }
