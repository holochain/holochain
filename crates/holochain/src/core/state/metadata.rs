...
impl<'env> MetadataBufT for MetadataBuf<'env> {
    ...
    #[allow(clippy::needless_lifetimes)]
    async fn register_update(&mut self, update: header::EntryUpdate) -> DatabaseResult<()> {
        let basis_entry_hash = update.basis_entry_hash.clone();
        self.register_header_on_basis(AnyDhtHash::from(basis_entry_hash.clone()), update)
            .await?;
        self.update_entry_dht_status(basis_entry_hash)?;
        Ok(())
    }
