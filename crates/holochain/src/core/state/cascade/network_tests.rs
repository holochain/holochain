...
async fn get_from_another_agent() {
    ...
    // Bob store element
    let post = Post("Bananas are good for you".into());
    let entry_hash = {
        ...
        let entry: Entry = post.clone().try_into().unwrap();
        let to_hash = entry.clone();
        let entry_hash: EntryHash = tokio_safe_block_on::tokio_safe_block_forever_on(async move {
            holochain_types::entry::EntryHashed::from_content(to_hash).await
        })
        .into_hash();
