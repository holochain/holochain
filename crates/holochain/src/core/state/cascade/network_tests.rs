...
async fn get_from_another_agent() {
    ...
    let entry_hash = EntryHash::with_data(&Entry::try_from(entry.clone()).unwrap());
    let header_hash = {
    ...
    assert_eq!(
        HeaderHash::with_data(entry_details.headers.get(0).unwrap()),
        header_hash
    );
    assert_eq!(
...
async fn get_links_from_another_agent() {
    ...
    let base_entry_hash = EntryHash::with_data(&Entry::try_from(base.clone()).unwrap());
    let target_entry_hash = EntryHash::with_data(&Entry::try_from(target.clone()).unwrap());
    ...
    assert_eq!(
        ...
        HeaderHash::with_data(&Header::LinkAdd(link_add))
    );
...
async fn generate_fixt_store() -> (
    ...
    let header = HeaderHashed::from_content(Header::EntryCreate(element_create));
