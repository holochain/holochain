...
pub fn sign_ephemeral(
    ...
    let mut seed = [0; 32];
    rng.fill(&mut seed).map_err(|e| WasmError::Guest(e.to_string()))?;
    let ephemeral_keypair = Ed25519KeyPair::from_seed_unchecked(&seed).map_err(|e| WasmError::Host(e.to_string()))?;

    let signatures: Result<Vec<Signature>, _> = input
        .into_inner()
        .into_iter()
        .map(|data| ephemeral_keypair
            .sign(&data)
            .as_ref()
            .try_into()
        ).collect();
...
pub mod wasm_test {
    ...
    async fn ribosome_sign_ephemeral_test() {
        ...
        let output: Vec<EphemeralSignatures> = crate::call_test_ribosome!(
            host_access,
            TestWasm::Sign,
            "sign_ephemeral",
            ()
        ...
        assert!(
            output[0].key.verify_signature_raw(
        ...
        assert!(
            output[1].key.verify_signature_raw(
