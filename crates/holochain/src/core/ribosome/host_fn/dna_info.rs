    ...
    match HostFnAccess::from(&call_context.host_context()) {
        HostFnAccess{ bindings_deterministic: Permission::Allow, .. } => {
            Ok(DnaInfo {
                ...
                properties: YamlProperties::try_from(ribosome.dna_def().properties.clone())?.into_inner(),
                zome_names: ribosome
                    .dna_def()
                    .zomes
                    .iter()
...
pub mod test {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn dna_info_prop_test() {
        observability::test_run().ok();
        let (dna_file, _) = SweetDnaFile::from_test_wasms(
            random_uid(),
            vec![TestWasm::ZomeInfo],
            YamlProperties::new(serde_yaml::from_str("foo: bar").unwrap()).try_into().unwrap()
        )
            .await
            .unwrap();
        ...
        let dna_info_foo: serde_yaml::Value = conductor.call(&alice, "dna_info_foo", ()).await;
        assert_eq!(dna_info_foo, serde_yaml::Value::String("bar".into()));
    }
}