...
use holochain_zome_types::CryptoBoxInput;
use holochain_zome_types::CryptoBoxOutput;
use std::sync::Arc;

pub fn crypto_box(
    _ribosome: Arc<impl RibosomeT>,
    call_context: Arc<CallContext>,
    input: CrypboBoxInput,
) -> RibosomeResult<CryptoBoxOutput> {
    Ok(SignOutput::new(
        tokio_safe_block_on::tokio_safe_block_forever_on(async move {
            call_context
                .host_access
                .keystore()
                .crypto_box(input.into_inner())
                .await
        })?,
    ))
}
