...
pub fn get_links_batch<'a>(
    _ribosome: Arc<impl RibosomeT>,
    call_context: Arc<CallContext>,
    inputs: Vec<GetLinksBatchInput>,
        ...
        HostFnAccess {
            ...
            let results: Vec<Result<Vec<Link>, RibosomeError>> =
                tokio_helper::block_forever_on(async move {
                    // This should be necessary but the compiler wants a `move` on the iterator map function
                    // and complains that batch_size and batch_index are borrowed. But `input` is owned by the closure.
                    // Fairly sure this is a bug, working around it with `move` and more references to the `call_context`
                    let call_ctx = std::iter::from_fn(|| Some(call_context.clone()));
                    futures::stream::iter(std::iter::zip(inputs.into_iter(), call_ctx).map(|(input, call_context)| async move {
                        let GetLinksBatchInput {
                            base_address,
                            link_type,
                            batch_size,
                            batch_index,
                        ...
                        .dht_get_links(key, GetLinksOptions {
