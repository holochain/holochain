...
mod test {
    ...
    fn zome() -> InlineZome {
        InlineZome::new_unique(vec![]).callback("close_chain", move |api, new_dna_hash: DnaHash| {
            let hash = api.close_chain(new_dna_hash)?;
            Ok(hash)
        })
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn can_close_source_chain() {
        ...
        let apps = conductor.setup_app("app", &[dna1.clone()]).await.unwrap();
        let (cell,) = apps.into_tuple();

        let hash: HeaderHash = conductor
            .call(&cell.zome("zome1"), "close_chain", dna2.dna_hash().clone())
            .await;
