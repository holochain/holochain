                        ...
                        let found: bool = txn
                            .query_row(
                                ...
                                SELECT EXISTS(
                                    ...
                                    AND action_hash = :hash
                                    AND validation_status = :status
                                    AND (type = :update_content OR type = :update_commit)
                                )
                                ",
                                named_params! {
                                    ":basis": base,
...
fn register_updated_commit(a: TestData) -> (Vec<Db>, Vec<Db>, &'static str) {
    let original_op = DhtOp::StoreCommit(
        a.signature.clone(),
        a.original_action.clone().into(),
        Some(a.original_entry.clone().into()),
    );
    let op = DhtOp::RegisterUpdatedCommit(
        a.signature.clone(),
    ...
    (pre_state, expect, "register updated commit")
}
