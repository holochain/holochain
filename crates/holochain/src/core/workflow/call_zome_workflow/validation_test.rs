...
async fn direct_validation_test() {
    ...
    let alice_dna_id = DnaId::new(dna_file.dna_hash().to_owned(), alice_agent_id.clone());

    let (_tmpdir, _app_api, handle) =
        setup_app_with_names(alice_agent_id, vec![("test_app", vec![(dna_file, None)])]).await;

    run_test(alice_dna_id, handle.clone()).await;
...
async fn run_test(alice_dna_id: DnaId, handle: ConductorHandle) {
    // Valid update should work
    let zome_call_params =
        new_zome_call_params(&alice_dna_id, "update_entry", (), TestWasm::Update).unwrap();
    handle.call_zome(zome_call_params).await.unwrap().unwrap();

    // Valid update of a private entry should work
    let zome_call_params =
        new_zome_call_params(&alice_dna_id, "update_private_entry", (), TestWasm::Update).unwrap();
    handle.call_zome(zome_call_params).await.unwrap().unwrap();

    // Invalid update should fail work
    let zome_call_params =
        new_zome_call_params(&alice_dna_id, "invalid_update_entry", (), TestWasm::Update).unwrap();
