...
mod tests {
    ...
    /// 1. All ops that can contain entries are created with entries (StoreCommit, StoreEntry and RegisterUpdatedContent)
    ...
    /// 6. Every op that is received (StoreCommit and RegisterUpdatedContent) is checked to match the expected versions (entries removed)
    ...
    fn test_private_entries(num_agents: u32) {
        tokio_helper::block_forever_on(
            async {
                ...
                // We are only expecting Store Commit and Register Replaced By ops and nothing else
                let store_commit_count = Arc::new(AtomicU32::new(0));
                let register_replaced_by_count = Arc::new(AtomicU32::new(0));
                let register_updated_commit_count = Arc::new(AtomicU32::new(0));
                ...
                let expected = {
                    ...
                    let expected_op = DhtOp::StoreCommit(
                        sig,
                    ...
                    map.insert(op_hash, (expected_op, store_commit_count.clone()));
                    ...
                    let expected_op =
                        DhtOp::StoreCommit(sig.clone(), entry_update_action.clone().into(), None);
                    let op_hash = expected_op.to_hash();
                    ...
                    let expected_op = DhtOp::RegisterUpdatedCommit(
                        sig.clone(),
                        entry_update_action.clone(),
                        None,
                    );
                    ...
                    map.insert(
                        op_hash,
                        (expected_op, register_updated_commit_count.clone()),
                    );
                ...
                assert_eq!(
                    num_agents * 1,
                    register_updated_commit_count.load(Ordering::SeqCst)
                );
                assert_eq!(num_agents * 2, store_commit_count.load(Ordering::SeqCst));
                assert_eq!(
                    num_agents * 2,
