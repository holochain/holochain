...
async fn validate_op_with_no_dependency() {
    ...
    let op_hash = test_case.save_op_to_db(
...
async fn validate_op_with_dependency_held_in_cache() {
    ...
    let previous_action = test_case.sign_action(Action::AgentValidationPkg(validation_package_action.clone()))
        .await;
    let previous_op = DhtOp::RegisterAgentActivity(fixt!(Signature), Action::AgentValidationPkg(validation_package_action));
    test_case.save_op_to_db(
        test_case.cache_db_handle(),
        previous_op,
    ).await.unwrap();
...
async fn validate_op_with_dependency_not_held() {
    ...
    network.expect_get().return_once(move |_, _| Ok(vec![response]));
...
impl TestCase {
    ...
    async fn run(&mut self) {
        ...
        let actual_network = self.actual_network.take().unwrap_or_else(|| MockHolochainP2pDnaT::new());
    ...
    async fn get_ops_pending_app_validation(&self) -> HashSet<DhtOpHash> {
        get_ops_to_app_validate(&self.dht_db_handle().into()).await.unwrap().into_iter().map(|op_hashed| op_hashed.hash).collect()
    }

    async fn expect_app_validation_triggered(&mut self) {
        tokio::time::timeout(std::time::Duration::from_secs(3), self.app_validation_trigger.1.listen()).await.expect("Timed out waiting for app validation to be triggered").unwrap();
    }
}
