mod get_zomes_to_invoke {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn get_zomes_to_invoke_register_record_with_entry() {
        let zomes = SweetInlineZomes::new(vec![], 0);
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn get_zomes_to_invoke_register_record_with_wrong_entry() {
        let zomes = SweetInlineZomes::new(vec![], 0);
        let (dna_file, integrity_zomes, _) = SweetDnaFile::unique_from_inline_zomes(zomes).await;
        let ribosome = RealRibosome::empty(dna_file.clone());
        let entry = fixt!(Entry);
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn get_zomes_to_invoke_register_record_without_entry_delete() {
        let zomes = SweetInlineZomes::new(vec![], 0);
        let (dna_file, integrity_zomes, _) = SweetDnaFile::unique_from_inline_zomes(zomes).await;
        let ribosome = RealRibosome::empty(dna_file.clone());
        ...
        let zomes = SweetInlineZomes::new(vec![], 0).integrity_function("validate", {
            move |api, op: Op| {
                if let Op::RegisterDelete(RegisterDelete { delete }) = op {
                    let result = api
                        .must_get_action(MustGetActionInput(delete.hashed.deletes_address.clone()));
                    if result.is_ok() {
    ...
    impl TestCase {
        async fn new(zomes: SweetInlineZomes) -> Self {
            let (dna_file, integrity_zomes, _) =
                SweetDnaFile::unique_from_inline_zomes(zomes).await;
