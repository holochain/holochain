...
use super::DnaError;
use super::{error::DnaResult, wasm::DnaWasm};
...
// NB: unfortunately it was necessary to make this a function rather than
// a From impl due to the orphan rule
pub fn test_wasm_to_pair(wasm: TestWasm) -> (ZomeName, ZomeDef) {
    Zome::from(wasm).into_inner()
}
...
impl From<TestWasm> for ZomeDef {
    fn from(test_wasm: TestWasm) -> Self {
        tokio_safe_block_on::tokio_safe_block_forever_on(async move {
            ...
            let (_, wasm_hash) = crate::nucleus::dna::wasm::DnaWasmHashed::from_content(dna_wasm)
                .await
                .into_inner();
            WasmZome { wasm_hash }.into()
        })
...
// NB: can't have this due to orphan rule
