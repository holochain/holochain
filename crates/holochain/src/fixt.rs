...
use crate::core::workflow::unsafe_call_zome_workspace::CallZomeWorkspaceFactory;
use ::fixt::prelude::*;
...
fixturator!(
    CallZomeWorkspaceFactory;
    curve Empty {
        CallZomeWorkspaceFactory::null()
    };
    curve Unpredictable {
        CallZomeWorkspaceFactoryFixturator::new(Empty)
            .next()
            .unwrap()
    };
    curve Predictable {
...
fixturator!(
    ZomeCallHostAccess;
    constructor fn new(CallZomeWorkspaceFactory, KeystoreSender, HolochainP2pCell);
);
...
fixturator!(
    ...
    constructor fn new(CallZomeWorkspaceFactory);
);
