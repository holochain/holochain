...
async fn zome_with_no_entry_types_does_not_prevent_deletes() {
    ...
    let (dna_file, _, _) = SweetDnaFile::unique_from_test_wasms(vec![TestWasm::ValidateRejectAppTypes, TestWasm::Crd]).await;
...
async fn zome_with_no_link_types_does_not_prevent_delete_links() {
    ...
    let (dna_file, _, _) = SweetDnaFile::unique_from_test_wasms(vec![TestWasm::ValidateRejectAppTypes, TestWasm::Link]).await;
    let agent = SweetAgents::alice();
    ...
    let _: ActionHash = conductor.call(&cell.zome("link"), "delete_link", created).await;
}
