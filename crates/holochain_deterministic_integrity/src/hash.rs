//! Functions to generate standardized hashes of Holochain commits and
//! arbitrary bytes.
//!
...
/// also be used with [`must_get_action`] etc. to retreive a _specific_ commit from the DHT rather than the
/// oldest live commit.
/// However there is no way to _generate_ an action hash directly from an action from inside wasm.
/// [`Commit`] values (entry+action pairs returned by [`must_get_action`] etc.) contain prehashed action structs
...
/// The action hash can be extracted from the Commit as `commit.action_hashed().as_hash()`.
///
