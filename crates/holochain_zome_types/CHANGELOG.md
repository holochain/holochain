...
## \[Unreleased\]
- **BREAKING**: Rename `GetOptions` variants. `Content` becomes `Local` and is now consistently querying only locally available data. No network calls are made. This applies to `get` and `get_details`. `Latest` becomes `Network` and fetches from the network if the caller is not an authority of the requested Action or Entry.
