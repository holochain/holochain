---
default_semver_increment_mode: !pre_minor a-minor-release-test
---
# Changelog

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/). This project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## Unreleased

## 0.1.0-a-minor-release-test.9

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.8

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.7

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.6

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.5

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.4

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.3

## 0.1.0-a-minor-release-test.2

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.1.0-a-minor-release-test.1

This release is another test for the release workflow. The resulting release on crates.io will be yanked.

- Added handling of `hc` extensions. This allows for existing executables in the system whose names match `hc-<COMMAND>` to be executed with `hc <COMMAND>`.

## 0.1.0-a-minor-release-test.0

This release is just a test for the release workflow. The resulting release on crates.io will be yanked.

## 0.0.70

## 0.0.69

## 0.0.68

## 0.0.67

## 0.0.66

## 0.0.65

## 0.0.64

## 0.0.63

## 0.0.62

## 0.0.61

## 0.0.60

## 0.0.59

## 0.0.58

## 0.0.57

## 0.0.56

## 0.0.55

## 0.0.54

## 0.0.53

## 0.0.52

## 0.0.51

## 0.0.50

## 0.0.49

## 0.0.48

## 0.0.47

## 0.0.46

## 0.0.45

## 0.0.44

## 0.0.43

## 0.0.42

## 0.0.41

## 0.0.40

## 0.0.39

## 0.0.38

## 0.0.37

## 0.0.36

## 0.0.35

## 0.0.34

## 0.0.33

## 0.0.32

- Fixed broken links in Rust docs [\#1284](https://github.com/holochain/holochain/pull/1284)

## 0.0.31

## 0.0.30

## 0.0.29

## 0.0.28

## 0.0.27

## 0.0.26

## 0.0.25

## 0.0.24

## 0.0.23

## 0.0.22

## 0.0.21

## 0.0.20

## 0.0.19

## 0.0.18

## 0.0.17

## 0.0.16

## 0.0.15

## 0.0.14

## 0.0.13

## 0.0.12

## 0.0.11

## 0.0.10

## 0.0.9

## 0.0.8

## 0.0.7

- Added the `hc web-app` sub-command for bundling up a UI with a previously created hApp bundle.  It uses the same same behavior as `hc dna` and `hc app` to specify the .yaml manifest file.

## 0.0.6

## 0.0.5

## 0.0.4

## 0.0.3

## 0.0.2

### Removed

- temporarily removed `install_app` from `hc`: its not clear if we should restore yet as mostly should be using `install_app_bundle` [\#665](https://github.com/holochain/holochain/pull/665)
