...
- `hc export-ts-bindings` writes the TypeScript type declarations of the conductor's admin and app API (as used by [holochain-client-js](https://github.com/holochain/holochain-client-js)) to a directory, `./bindings` by default. Only present when `hc` is built with the opt-in `ts_rs` Cargo feature.
