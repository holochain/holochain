...
//!
//! NOTE: this was copied over from the `kitsune_p2p` crate and somewhat modified,
//! as part of the gossip round rewrite. The original can be eventually removed.

use std::collections::HashMap;
use std::collections::VecDeque;
use std::time::Duration;

use kitsune_p2p_dht::region::Region;
use kitsune_p2p_types::metrics::MetricRecord;
use kitsune_p2p_types::metrics::MetricRecordKind;
...
use crate::gossip_type::GossipModuleType;
use crate::PeerId;
...
pub struct Metrics {
    ...
    agent_history: HashMap<KAgent, PeerAgentHistory>,

    /// Map of remote nodes and gossip history with each
    node_history: HashMap<PeerId, PeerNodeHistory>,
...
impl<'lt> From<&'lt KAgent> for AgentLike<'lt> {
    fn from(pk: &'lt KAgent) -> Self {
        Self::PubKey(pk)
    }
}
...
impl Metrics {
    ...
    pub fn complete_current_round(&mut self, node: &PeerId, error: bool) {
    ...
    /// Getter
    pub fn peer_agent_histories(&self) -> &HashMap<KAgent, PeerAgentHistory> {
        &self.agent_history
    }

    /// Getter
    pub fn peer_node_histories(&self) -> &HashMap<PeerId, PeerNodeHistory> {
        &self.node_history
    }
}
