...
fn test_peer_coverage() {
    ...
    let arc = |c, n, h| {
        let mut arc = ArcInterval::from_start_and_halflen(0u32, h);
        arc.update_length(PeerViewBeta::new(Default::default(), arc, c, n));
        (arc.coverage() * 10000.0).round() / 10000.0
    };

    let converge = |arc: &mut ArcInterval, peers: &Vec<ArcInterval>| {
        for _ in 0..40 {
    ...
    let mut arc = ArcInterval::from_start_and_halflen(0u32, MAX_HALF_LENGTH / 2);
    ...
    let mut arc = ArcInterval::from_start_and_halflen(0u32, MAX_HALF_LENGTH);
    ...
    let mut arc = ArcInterval::from_start_and_halflen(u32::MAX / 2, MAX_HALF_LENGTH);
    ...
    let mut arc = ArcInterval::from_start_and_halflen(u32::MAX / 2, 0);
