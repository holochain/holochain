...
impl peer_store::PeerStoreFactory for MemPeerStoreFactory {
    fn create(&self, _builder: Arc<builder::Builder>) -> peer_store::DynPeerStore {
...
impl peer_store::PeerStore for MemPeerStore {
    ...
    fn get(&self, agent: DynId) -> BoxFuture<'_, Option<agent::DynAgentInfo>> {
    ...
    fn query_by_time_and_arq(
        ...
        let r = self.0.lock().unwrap().query_by_time_and_arq(since, until, arq);
        Box::pin(async move { r })
    }

    fn query_by_location(&self, loc: u32, limit: usize) -> BoxFuture<'_, Vec<agent::DynAgentInfo>> {
...
impl Inner {
    ...
    pub fn query_by_location(&mut self, basis: u32, limit: usize) -> Vec<agent::DynAgentInfo> {
        ...
        let mut out: Vec<(u32, &agent::DynAgentInfo)> = self.store
            .values()
            .filter_map(|v| {
                if v.is_active() {
...
mod test {
    ...
    async fn empty_query_by_loc() {
        let store = builder::Builder::create_test().create_peer_store();
        assert_eq!(0, store.query_by_location(0, 1).await.len());
    }
}
