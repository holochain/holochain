...
agent_hash(&[0]), ChainFilter::new(action_hash(&[6])).take(7).until_timestamp(Timestamp::from_micros(5 as i64 * 1000))
...
agent_hash(&[0]), ChainFilter::new(action_hash(&[6])).take(2).until_hash(action_hash(&[4])).until_timestamp(Timestamp::from_micros(3 as i64 * 1000))
...
agent_hash(&[0]), ChainFilter::new(action_hash(&[6])).take(7).until_hash(action_hash(&[5])).until_timestamp(Timestamp::from_micros(3 as i64 * 1000))
...
agent_hash(&[0]), ChainFilter::new(action_hash(&[6])).take(7).until_hash(action_hash(&[4])).until_timestamp(Timestamp::from_micros(5 as i64 * 1000))
...
async fn test_must_get_agent_activity_ok(
    ...
    test_must_get_agent_activity_inner(data, author, filter).await.unwrap()
}
...
async fn test_must_get_agent_activity_err(
    ...
    test_must_get_agent_activity_inner(data, author, filter).await.unwrap_err()
}
...
async fn test_must_get_agent_activity_1() {
    ...
    assert!(matches!(res, Ok(MustGetAgentActivityResponse::Activity {..})));
}
