...
FixtureDataStores {
    dht: FixtureData {
        // Canonical chain: seq 3, 4, 5 — all timestamps (3000, 4000, 5000 µs) above the
        // boundary (1500 µs).
        activity: vec![(
            agent_hash(&[0]),
            vec![
                TestChainItem::with_ts(5, 5000),
                TestChainItem::with_ts(4, 4000),
    ...
    cache: FixtureData {
        // Non-canonical forked action at seq 3 with timestamp 500 µs — below the
        // boundary. Before the fix this SQL witness would fire on this sibling action
        // and incorrectly report Complete. After the fix only merged_activity.last()
        // (the canonical seq-3 action, ts=3000) is checked, so the result stays
        // Indeterminate.
        activity: vec![(
            agent_hash(&[0]),
            vec![TestChainItem {
                seq: 3,
                hash: TestChainHash::forked(3, 1),
                timestamp: Timestamp::from_micros(500),
                prev: Some(TestChainHash(2)),
            }],
        )],
        ..Default::default()
    },
...
agent_hash(&[0]),
ChainFilter::until_timestamp(action_hash(&[5]), Timestamp::from_micros(1500))
=> MustGetAgentActivityResponse::UntilTimestampIndeterminate(Timestamp::from_micros(1500)); "non-canonical branch below boundary does not trigger Complete")]
#[test_case(
FixtureDataStores {
    dht:  FixtureData {
...
=> MustGetAgentActivityResponse::UntilTimestampIndeterminate(Timestamp::from_micros(7_i64 * 1000)); "until_timestamp indeterminate when canonical chain does not precede boundary")]
#[test_case(
FixtureDataStores {
    dht:  FixtureData {
...
agent_hash(&[0]), ChainFilter::until_timestamp(action_hash(&[9]), Timestamp::from_micros(7_i64 * 1000))
=> matches MustGetAgentActivityResponse::Activity { activity, .. } if activity.len() == 3; "until_timestamp complete when canonical chain precedes boundary")]
#[test_case(
FixtureDataStores {
    dht:  FixtureData {
        activity: agent_chain(&[(0, 0..7)]),
