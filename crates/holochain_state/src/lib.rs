...
//! - [`ElementBuf`](crate::prelude::ElementBuf): the union of two CasBuffers, one for Entries, one for Headers
...
//! - SourceChainBuf: the union of a [`ElementBuf`](crate::prelude::ElementBuf) and a [ChainSequenceBuf], which fully represents a source chain
//! - MetadataBuf: (*unimplemented*) Uses a KvvBuffer to represent EAV-like relationships between CAS entries
//! - Cascade: (*unimplemented*) Unifies two [`ElementBuf`](crate::prelude::ElementBuf) and two [MetadataBuf] references (one of each is a cache)
