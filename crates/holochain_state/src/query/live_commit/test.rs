...
async fn can_handle_update_in_scratch() {
    ...
    let query = GetLiveCommitQuery::with_private_data_access(
        td.update_hash,
        Arc::new(td.update_store_commit_op.action().author().clone()),
    );

    // - Create an entry on main db.
    insert_valid_integrated_op(&mut txn, &td.update_store_commit_op).unwrap();
    let r = query
        .run(Txn::from(&txn))
        .unwrap()
    ...
    insert_op_scratch(
        &mut scratch,
        td.update_store_commit_op.clone(),
