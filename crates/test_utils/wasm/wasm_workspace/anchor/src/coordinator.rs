...
fn anchor(input: AnchorInput) -> ExternResult<EntryHash> {
    hdk::prelude::anchor(Zomes::IntegrityAnchor, input.0, input.1)
}
...
fn list_anchor_type_addresses(_: ()) -> ExternResult<Vec<AnyLinkableHash>> {
    hdk::prelude::list_anchor_type_addresses(Zomes::IntegrityAnchor)
}

#[hdk_extern]
fn list_anchor_addresses(anchor_type: String) -> ExternResult<Vec<AnyLinkableHash>> {
    hdk::prelude::list_anchor_addresses(Zomes::IntegrityAnchor, anchor_type)
}

#[hdk_extern]
fn list_anchor_tags(anchor_type: String) -> ExternResult<Vec<String>> {
    hdk::prelude::list_anchor_tags(Zomes::IntegrityAnchor, anchor_type)
}
