...
fn anchor(input: AnchorInput) -> ExternResult<EntryHash> {
    hdk::prelude::anchor(LinkType::try_from(LinkTypes::Anchor)?, input.0, input.1)
}
...
fn anchor_many(inputs: ManyAnchorInput) -> ExternResult<Vec<EntryHash>> {
    ...
    for input in inputs.0 {
        out.push(hdk::prelude::anchor(
            LinkType::try_from(LinkTypes::Anchor)?,
            input.0,
            input.1,
        )?);
    }
...
fn list_anchor_type_addresses(_: ()) -> ExternResult<Vec<AnyLinkableHash>> {
    hdk::prelude::list_anchor_type_addresses(LinkType::try_from(LinkTypes::Anchor)?)
}

#[hdk_extern]
fn list_anchor_addresses(anchor_type: String) -> ExternResult<Vec<AnyLinkableHash>> {
    hdk::prelude::list_anchor_addresses(LinkType::try_from(LinkTypes::Anchor)?, anchor_type)
}

#[hdk_extern]
fn list_anchor_tags(anchor_type: String) -> ExternResult<Vec<String>> {
    hdk::prelude::list_anchor_tags(LinkType::try_from(LinkTypes::Anchor)?, anchor_type)
}
