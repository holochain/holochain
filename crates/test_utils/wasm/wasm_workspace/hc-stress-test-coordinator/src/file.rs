...
pub fn get_file(original_file_hash: ActionHash) -> ExternResult<Option<Record>> {
    let links = get_links(
        LinkQuery::try_new(original_file_hash.clone(), LinkTypes::FileUpdates)?,
        GetStrategy::default(),
    )?;
...
pub fn update_file(input: UpdateFileInput) -> ExternResult<Record> {
    let updated_file_hash = update_entry(input.previous_file_hash.clone(), &input.updated_file)?;
    create_link(
    ...
    let record = get(updated_file_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
