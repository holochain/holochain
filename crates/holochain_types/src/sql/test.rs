...
#[test_case(LinkTypeRange::Empty => " AND false ".to_string())]
#[test_case(LinkTypeRange::Full => "".to_string())]
#[test_case(LinkTypeRange::Inclusive(LinkType(0)..=LinkType(0))=> " AND link_type = 0 ".to_string())]
#[test_case(LinkTypeRange::Inclusive(LinkType(1)..=LinkType(0))=> " AND false ".to_string())]
#[test_case(LinkTypeRange::Inclusive(LinkType(5)..=LinkType(6))=> " AND link_type BETWEEN 5 AND 6 ".to_string())]
...
#[test_case(make_ranges(vec![0..=u8::MAX, 1..=1, 5..=5]) => " AND link_type = 1  AND link_type = 5 ".to_string())]
#[test_case(make_ranges(vec![0..=u8::MAX, 1..=1, 5..=6]) => " AND link_type = 1  AND link_type BETWEEN 5 AND 6 ".to_string())]
#[test_case(make_ranges(vec![0..=5, 30..=50, 7..=9]) => " AND link_type BETWEEN 0 AND 5  AND link_type BETWEEN 30 AND 50  AND link_type BETWEEN 7 AND 9 ".to_string())]
