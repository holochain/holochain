...
impl DnaFile {
    ...
    pub fn modify_phenotype(&self, network_seed: NetworkSeed, properties: YamlProperties) -> DnaResult<Self> {
        let mut clone = self.clone();
        clone.dna = DnaDefHashed::from_content_sync(
            clone.dna.modify_phenotype(network_seed, properties.try_into()?),
        );
        Ok(clone)
    }
}
