...
mod current;
pub(crate) mod web_app_manifest_v0;
...
impl WebAppManifest {
    ...
    pub fn current(name: String) -> Self {
        WebAppManifest::V0(WebAppManifestV0 {
            name,
            ui: WebUI {
    ...
    pub fn app_name(&self) -> &str {
        match self {
            Self::V0(WebAppManifestV0 { name, .. }) => name,
        }
    }
    ...
    pub fn web_ui_location(&self) -> ResourceIdentifier {
        match self {
            Self::V0(WebAppManifestV0 { ui, .. }) => resource_id_for_ui(&ui.path),
        }
    }
    ...
    pub fn happ_bundle_location(&self) -> ResourceIdentifier {
        match self {
            Self::V0(WebAppManifestV0 { happ, .. }) => resource_id_for_happ(&happ.path),
        }
    }
}
...
mod tests {
    use crate::web_app::{
        web_app_manifest::WebAppManifestV0, AppManifestLocation, WebAppManifest, WebUI,
    };
    ...
    fn web_app_manifest_v0_helper_functions() {
        ...
        let web_app_manifest = WebAppManifest::V0(WebAppManifestV0 {
            name: app_name.clone(),
            ui: WebUI {
