...
impl<'a> ElementGroup<'a> {
    ...
    pub async fn from_wire_elements<I: IntoIterator<Item = WireNewEntryHeader>>(
        ...
        let entry = EntryHashed::from_content(entry);
...
pub trait SignedHeaderHashedExt {
    ...
    fn from_content(signed_header: SignedHeader) -> SignedHeaderHashed;
    /// Sign some content
    async fn new(
...
impl SignedHeaderHashedExt for SignedHeaderHashed {
    fn from_content(signed_header: SignedHeader) -> Self
    where
        Self: Sized,
    {
        let (header, signature) = signed_header.into();
        Self::with_presigned(HeaderHashed::from_content(header), signature)
    }
...
impl WireElement {
    ...
    pub async fn into_element_and_delete(self) -> (Element, Option<Element>) {
        let header = Element::new(
            SignedHeaderHashed::from_content(self.signed_header),
            self.maybe_entry,
        );
...
mod tests {
    ...
    async fn test_signed_header_roundtrip() {
        ...
        let hashed: HoloHashed<SignedHeader> = HoloHashed::from_content(signed_header);
