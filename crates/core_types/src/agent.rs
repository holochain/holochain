...
impl AgentId {
    ...
    pub fn new_with_raw_key(nick: &str, key: &str) -> SkunkResult<Self> {
        let codec = HcidEncoding::with_kind("hcs0").map_err(|e| e.to_string())?;
        let key_b32 = codec.encode(key.as_bytes()).map_err(|e| e.to_string())?;
    ...
    pub fn decoded_key(&self) -> SkunkResult<String> {
        let codec = HcidEncoding::with_kind("hcs0").map_err(|e| e.to_string())?;
        let key_b32 = codec.decode(&self.pub_sign_key).map_err(|e| e.to_string())?;
...
//         match Entry::try_from(content).map_err(|e| e.to_string())? {
