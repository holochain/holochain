...
#[tokio::test]
async fn retrieve_ops_does_not_panic_with_invalid_op_ids() {
    enable_tracing();
    let (_, op_store) = setup_test().await;

    let op_id_too_long = kitsune2_api::OpId::from(bytes::Bytes::from(vec![0u8; 55]));
    assert_eq!(op_id_too_long.len(), 55);

    let retrieved_ops = op_store.retrieve_ops(vec![op_id_too_long]).await.unwrap();
    assert_eq!(retrieved_ops.len(), 0);

    let op_id_too_short = kitsune2_api::OpId::from(bytes::Bytes::from(vec![0u8; 12]));
    assert_eq!(op_id_too_short.len(), 12);

    let retrieved_ops_2 = op_store.retrieve_ops(vec![op_id_too_short]).await.unwrap();
    assert_eq!(retrieved_ops_2.len(), 0);
}
...
async fn filter_out_existing_ops_filters_invalid_op_ids_as_well() {
    ...
    let out = op_store
        .filter_out_existing_ops(vec![op_id_too_long, valid_op_id.clone()])
        .await
        .unwrap();

    assert_eq!(out.len(), 1);
    assert_eq!(out[0].clone(), valid_op_id.clone());
    ...
    assert_eq!(out_2[0].clone(), valid_op_id);
}
