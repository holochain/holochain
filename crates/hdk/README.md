...
- `fn recv_remote_signal(signal: ExternIO) -> ExternResult<()>`:
  ...
  - This is a regular zome function, so in order for remote agents to send signals to this zome, a capability grant (e.g., with `GrantConstraint::Unrestricted`) must be created for the appropriate agent(s).
