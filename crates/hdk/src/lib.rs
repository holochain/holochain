...
//! - Exposing functions to external processes and callbacks to the host: [`hdk_extern!`](crate::prelude::hdk_extern!) and [`map_extern!`](crate::prelude::map_extern!) macros
...
//! ## Extern callbacks
//!
//! To extend a Rust function so that it can be called by the host, add the [`hdk_extern!`](crate::prelude::hdk_extern!) attribute.
//!
...
//! These are the 'high level' functions such as [`crate::entry::create_entry`] and macros like [`hdk_extern!`](crate::prelude::hdk_extern!).
...
//!
//! Every extern defined with the [`hdk_extern!`](crate::prelude::hdk_extern!) attribute registers a [tracing subscriber](https://crates.io/crates/tracing-subscriber) that works in WASM.
//!
