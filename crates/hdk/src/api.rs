...
macro_rules! api_call {
    ( $f:ident, $input:expr, $outputt:ty ) => {{
        ...
        let result: Result<$outputt, $crate::prelude::SerializedBytesError> = $crate::prelude::host_call!(
            $f,
            $input
        );
...
macro_rules! commit_entry {
    ( $input:expr ) => {{
        ...
        match try_sb {
            Ok(sb) => $crate::api_call!(__commit_entry, CommitEntryInput::new(($input.into(), Entry::App(sb))), CommitEntryOutput),
            Err(e) => Err(e),
        }
    }};
}
...
macro_rules! entry_hash {
    ( $input:expr ) => {{
        ...
        let try_sb = SerializedBytes::try_from($input);
        match try_sb {
            Ok(sb) => $crate::api_call!(__entry_hash, EntryHashInput::new(Entry::App(sb)), EntryHashOutput),
            Err(e) => Err(e),
        }
    }};
}
