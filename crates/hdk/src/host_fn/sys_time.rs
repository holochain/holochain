...
///
/// so this is an improvement on pushing time collection back onto the "client" which can't
/// guarantee that it is seeing the same time as the rust host so validation logic can't be so
/// strict in a safe way.
///
/// @todo
...
/// note that the **redundantly fetches** and **cross-references** them is critical, failure to do
/// this brought down the ETH2.0 testnet due to a single roughtime server from cloudflare being 24
/// hours off.
