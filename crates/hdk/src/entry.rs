...
///
/// The host builds a [`Create`] action for the passed entry value and commits to the chain.
///
...
///
/// The host builds a [`Delete`] action for the passed entry and commits to the chain.
///
...
///
/// This function accepts the [`ActionHash`] of the commit to mark deleted and optionally an argument to
...
///
/// The hash is the [`ActionHash`] of the deleted commit, the input is a [`TryInto<CreateInput>`].
///
/// Updates can reference Commits which contain Entry data -- namely, Creates and other Updates -- but
/// not Deletes or system Commits.
///
/// As updates can reference commits on other agent's source chains across unpredictable network
...
/// author them against that commit. It is up to happ developers to decide how to ensure the tree
/// branches are walked appropriately and that updates point to the correct commit, whatever that
/// means for the happ.
///
/// ```ignore
/// #[hdk_entry(id = "foo")]
...
/// Gets a commit for a given entry or action hash.
///
/// The behaviour of get changes subtly per the _type of the passed hash_.
/// An action hash returns the commit for that action, i.e. action+entry or action+None.
/// An entry hash returns the "oldest live" commit, i.e. action+entry.
///
/// An commit is no longer live once it is referenced by a valid delete commit.
/// An update to a commit does not change its liveness.
/// See [`get_details`] for more information about how CRUD commits reference each other.
///
...
///            "foo" as part of a commit with the "oldest live" action.
...
///            creates and deletes allowed by validation rules, the "oldest live" commit _does_
///            represent the commit pointing at the first agent to claim a name, but it could also
...
///       be live.
pub fn get<H>(hash: H, options: GetOptions) -> ExternResult<Option<Commit>>
where
    AnyDhtHash: From<H>,
{
    Ok(HDK
...
/// Get a commit and its details for the entry or action hash passed in.
/// Returns [`None`] if the entry/action does not exist.
/// The details returned are a contextual mix of commits and action hashes.
///
...
///
/// Note: If an action hash is passed in the commit returned is the specified commit.
///       If an entry hash is passed in all the actions (so implicitly all the commits) are
...
///
/// Creates are initial action/entry combinations (commits) produced by commit_entry! and cannot
...
///
/// Full commits are returned for direct references to the passed hash.
/// Action hashes are returned for references to references to the passed hash.
///
/// [`Details`] for an action hash return:
/// - the commit for this action hash if it exists
/// - all update and delete _commits_ that reference that specified action
///
/// [`Details`] for an entry hash return:
/// - all creates, updates and delete _commits_ that reference that entry hash
/// - all update and delete _commits_ that reference the commits that reference the entry hash
///
...
///
/// Note: There are multiple action types that exist and operate entirely outside of CRUD commits
...
///       However the commit will still be returned by [`get_details`] if an action hash is passed,
///       these non-entry commits will have [`None`] as the entry value.
