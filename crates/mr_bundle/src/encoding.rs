...
pub fn pack<T: serde::ser::Serialize>(data: &T) -> MrBundleResult<bytes::Bytes> {
    ...
    enc.write_all(&bytes).map_err(|e| {
        MrBundleError::IoError(
            "Failed to compress bundle".to_string(),
            e,
        )
    })?;
    Ok(enc.finish().map_err(|e| {
        MrBundleError::IoError(
            "Failed to finish compressing bundle".to_string(),
            e,
        )
    })?.into())
}
...
pub fn unpack<T: serde::de::DeserializeOwned>(compressed: impl Read) -> MrBundleResult<T> {
    ...
    gz.read_to_end(&mut bytes).map_err(|e| {
        MrBundleError::IoError(
            "Failed to decompress bundle".to_string(),
            e,
        )
    })?;
