...
# Runs the export binary in holochain_conductor_api, which stages the whole
# binding tree in one process (ts-rs only merges declarations sharing an
# output file within a single process) before atomically replacing
# TS_RS_EXPORT_DIR. See holochain_conductor_api::export_ts_bindings.
ts-bindings:
	TS_RS_EXPORT_DIR=$(or $(TS_RS_EXPORT_DIR),./bindings) \
		cargo run -p holochain_conductor_api --features ts_rs,unstable-countersigning --bin export-ts-bindings

# The crate's ts_rs-gated tests: the wire-format smoke test and the
# tag-injection helper's unit tests.
